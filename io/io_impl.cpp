...
#include "io/easykey.hpp"
#include "io/easywind.hpp"
...
std::ostream& operator<<(std::ostream& os, const MidiMessage& msg) {
    switch( msg.type_ ) {
        case MidiMessage::NoteOn:
            os << "Note On " << (int)msg.dd.d1_ << " " << (int)msg.dd.d2_ ;
            break;
        case MidiMessage::NoteOff:
            os << "Note Off " << (int)msg.dd.d1_ << " " << (int)msg.dd.d2_ ;
            break;
        case MidiMessage::ControlChange:
            os << "CC " << (int)msg.dd.d1_ << " " << (int)msg.dd.d2_ ;
            break;
        case MidiMessage::PitchWhell:
            os << "PitchWhell " << (int)msg.d.d_ ;
            break;
        default:
            os << "Unknow " ;
    }
...
void init_words(Enviroment& env) {
    ...
    env.insert_native_word("io.ezwind", EasyWindController::creator);
    env.insert_native_word("io.monokey", EasyMonoKeyboard::creator);
}
