...
OsSawtoothWord::~OsSawtoothWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void OsSawtoothWord::run(Stack& stack) {
    const int nInput = 0;
...
void OsSawtoothWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void OsSawtoothWord::signals_input(Stack& stack) {
    float din_[0];
    float* din[0];
...
void OsSawtoothWord::vectors_input(Stack& stack) {
    const Vec* din_[0];
    float* din[0];
    for (int i =  0; i < 0; i++) {
