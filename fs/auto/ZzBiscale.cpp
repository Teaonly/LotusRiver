...
ZzBiscaleWord::~ZzBiscaleWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ZzBiscaleWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    // load parameters
    *up_ = stack.pop_number();
    *low_ = stack.pop_number();
...
void ZzBiscaleWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ZzBiscaleWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void ZzBiscaleWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
