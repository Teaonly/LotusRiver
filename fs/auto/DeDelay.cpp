...
DeDelayWord::~DeDelayWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void DeDelayWord::run(Stack& stack) {
    const int nInput = 1;
...
void DeDelayWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void DeDelayWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void DeDelayWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
