...
ReMonoFreeverbWord::~ReMonoFreeverbWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ReMonoFreeverbWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    if ( dsp == nullptr ) {
        ...
        dsp = new mydsp::ReMonoFreeverb();
        dsp->init(sr);
        dsp->buildUserInterface(&ui);

        // init parameters
        spread_ = ui.value("spread");
        damp_ = ui.value("damp");
        fb2_ = ui.value("fb2");
        fb1_ = ui.value("fb1");
    ...
    // load parameters
    *spread_ = stack.pop_number();
    *damp_ = stack.pop_number();
    *fb2_ = stack.pop_number();
    *fb1_ = stack.pop_number();
...
void ReMonoFreeverbWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ReMonoFreeverbWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void ReMonoFreeverbWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
