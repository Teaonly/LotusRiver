...
ZzMetroWord::~ZzMetroWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ZzMetroWord::run(Stack& stack) {
    const int nInput = 0;
...
void ZzMetroWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ZzMetroWord::signals_input(Stack& stack) {
    float din_[0];
    float* din[0];
...
void ZzMetroWord::vectors_input(Stack& stack) {
    const Vec* din_[0];
    float* din[0];
    for (int i =  0; i < 0; i++) {
