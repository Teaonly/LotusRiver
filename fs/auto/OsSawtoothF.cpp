...
OsSawtoothFWord::~OsSawtoothFWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void OsSawtoothFWord::run(Stack& stack) {
    const int nInput = 1;
...
void OsSawtoothFWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void OsSawtoothFWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void OsSawtoothFWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
