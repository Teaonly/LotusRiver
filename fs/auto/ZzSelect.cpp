...
ZzSelectWord::~ZzSelectWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ZzSelectWord::run(Stack& stack) {
    const int nInput = 3;
...
void ZzSelectWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ZzSelectWord::signals_input(Stack& stack) {
    float din_[3];
    float* din[3];
...
void ZzSelectWord::vectors_input(Stack& stack) {
    const Vec* din_[3];
    float* din[3];
    for (int i =  0; i < 3; i++) {
