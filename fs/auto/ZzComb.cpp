...
ZzCombWord::~ZzCombWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ZzCombWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    if ( dsp == nullptr ) {
        ...
        dsp = new mydsp::ZzComb();
        dsp->init(sr);
        dsp->buildUserInterface(&ui);

        // init parameters
        coef_x_ = ui.value("coef_x");
        coef_y_ = ui.value("coef_y");
        rtime_ = ui.value("rtime");

    }
    
    // load parameters
    *coef_x_ = stack.pop_number();
    *coef_y_ = stack.pop_number();
    *rtime_ = stack.pop_number();
...
void ZzCombWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ZzCombWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void ZzCombWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
