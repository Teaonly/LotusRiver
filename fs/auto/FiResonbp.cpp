...
FiResonbpWord::~FiResonbpWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void FiResonbpWord::run(Stack& stack) {
    const int nInput = 1;
...
void FiResonbpWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void FiResonbpWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void FiResonbpWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
