...
FiHighpass3eWord::~FiHighpass3eWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void FiHighpass3eWord::run(Stack& stack) {
    const int nInput = 1;
...
void FiHighpass3eWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void FiHighpass3eWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void FiHighpass3eWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
