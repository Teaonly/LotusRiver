...
EnAdsrWord::~EnAdsrWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void EnAdsrWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    if ( dsp == nullptr ) {
        ...
        dsp = new mydsp::EnAdsr();
        dsp->init(sr);
        dsp->buildUserInterface(&ui);

        // init parameters
        release_ = ui.value("release");
        sustain_ = ui.value("sustain");
        decay_ = ui.value("decay");
    ...
    *sustain_ = stack.pop_number();
    *decay_ = stack.pop_number();
...
void EnAdsrWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void EnAdsrWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void EnAdsrWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
