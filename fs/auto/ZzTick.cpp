...
ZzTickWord::~ZzTickWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void ZzTickWord::run(Stack& stack) {
    const int nInput = 0;
...
void ZzTickWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void ZzTickWord::signals_input(Stack& stack) {
    float din_[0];
    float* din[0];
...
void ZzTickWord::vectors_input(Stack& stack) {
    const Vec* din_[0];
    float* din[0];
    for (int i =  0; i < 0; i++) {
