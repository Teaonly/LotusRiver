...
Compilation options: -lang cpp -light -cn FiResonbp -es 1 -mcd 16 -single -ftz 0
------------------------------------------------------------ */

#ifndef  __FiResonbp_H__
#define  __FiResonbp_H__
...
static float FiResonbp_faustpower2_f(float value) {
	return value * value;
}

class FiResonbp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-lang cpp -light -cn FiResonbp -es 1 -mcd 16 -single -ftz 0");
		m->declare("filename", "FiResonbp.dsp");
	...
	virtual FiResonbp* clone() {
		return new FiResonbp();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("FiResonbp");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = 2.0f * (1.0f - 1.0f / FiResonbp_faustpower2_f(fSlow0));
		...
		float fSlow7 = float(fEntry2) / (fSlow0 * fSlow5);
		float fSlow8 = 0.0f - fSlow7;
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = float(input0[i0]) - fSlow6 * (fSlow4 * fRec0[2] + fSlow1 * fRec0[1]);
			output0[i0] = FAUSTFLOAT(fSlow7 * fRec0[0] + fSlow8 * fRec0[2]);
			fRec0[2] = fRec0[1];
			fRec0[1] = fRec0[0];
		}
	}
