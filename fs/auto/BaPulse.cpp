...
BaPulseWord::~BaPulseWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void BaPulseWord::run(Stack& stack) {
    const int nInput = 0;
...
void BaPulseWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void BaPulseWord::signals_input(Stack& stack) {
    float din_[0];
    float* din[0];
...
void BaPulseWord::vectors_input(Stack& stack) {
    const Vec* din_[0];
    float* din[0];
    for (int i =  0; i < 0; i++) {
