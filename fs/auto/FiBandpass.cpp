...
FiBandpassWord::~FiBandpassWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void FiBandpassWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    // load parameters
    *fu_ = stack.pop_number();
    *fl_ = stack.pop_number();
...
void FiBandpassWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void FiBandpassWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void FiBandpassWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
