...
BaBitcrusherWord::~BaBitcrusherWord() {
    if ( dsp != nullptr ) {
        delete dsp;
    }
}

void BaBitcrusherWord::run(Stack& stack) {
    const int nInput = 1;
    ...
    if ( dsp == nullptr ) {
        ...
        dsp = new mydsp::BaBitcrusher();
        dsp->init(sr);
        dsp->buildUserInterface(&ui);

        // init parameters
        nbits_ = ui.value("nbits");

    }
    
    // load parameters
    *nbits_ = stack.pop_number();
...
void BaBitcrusherWord::none_input(const int bs, Stack& stack) {
    float* dout[1];
...
void BaBitcrusherWord::signals_input(Stack& stack) {
    float din_[1];
    float* din[1];
...
void BaBitcrusherWord::vectors_input(Stack& stack) {
    const Vec* din_[1];
    float* din[1];
    for (int i =  0; i < 1; i++) {
