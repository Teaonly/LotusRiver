...
namespace base {
    ...
    struct Zeros : public StaticNativeWord {
        virtual void run_first(Stack& stack) {
            ...
            lr_assert(len > 1, "Can't creat scalar like vector!");
            vec = Vec::Zero(len, 1);
...
namespace math {
    ...
    struct Clip : public NativeWord {
        virtual void run(Stack& stack) {
            auto max = stack.pop_number();
            auto min = stack.pop_number();

            if ( stack.top().is_number() ) {
                auto v = stack.pop_number();
                if ( v > max ) {
                    v = max;
                }
                if ( v < min ) {
            ...
            if ( result.size() != a->size() ) {
                result = Vec::Zero(a->rows(), a->cols());
            }
            const float* vin = a->data();
            float *vout = result.data();
        ...
        NWORD_CREATOR_DEFINE_LR(Clip);
    private:
        Vec result;
    };

    struct Comp : public NativeWord {
        virtual void run(Stack& stack) {
            if ( stack.top().is_number() ) {
                auto a = stack.pop_number();
                auto b = stack.pop_number();
            ...
            auto b = stack.pop_vector();
            if ( a->size() != b->size() ) {
                lr_panic("Comp can't campare two vector with diff size");
            }
            if ( result.size() != a->size() ) {
                result = Vec::Zero(a->rows(), a->cols());
            }
...
void Enviroment::load_base_math() {
    ...
    insert_native_word("clip", math::Clip::creator );
    insert_native_word("comp", math::Comp::creator );

    insert_native_word("math.pi", math::PI::creator );
    insert_native_word("math.e", math::E::creator );
}

Runtime Enviroment::runtime(const std::string& txt) {
    auto main_code = compile(txt);
    Runtime rt(*this, main_code);
    return rt;
}

Runtime* Enviroment::new_runtime(const std::string& txt) {
    auto main_code = compile(txt);
    Runtime* rt = new Runtime(*this, main_code);
    return rt;
}
